...
application : iliad , batch  ;
pour i=V,C,1..4:
TTSPRi = (TTSNNi + TPRRi) * (1-ART1731BIS) + max(0,TTSNNi + TPRRi) * ART1731BIS;
pour i=V,C,1..4:
...
TEFFREV =   INDTEFF *
                  ...
                  + ESFP + TTSPR + RCM + RRFI +PLOCNETF + NPLOCNETF
                  + BPVRCM + PVTAXSB + BPVKRI + (PVIMPOS+COD3WI) * (1-null(1 -FLAG_EXIT)) + COD3VE + PVSURSI * null(2 -FLAG_EXIT)
                  + max(BANOR,0) + REB +
                  min(BANOR,0) *
