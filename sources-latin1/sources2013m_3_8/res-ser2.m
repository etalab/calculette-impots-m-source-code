...
IND61PS =  (positif_ou_nul((TOTCR - CSGIM - CRDSIM - PRSPROV - CDISPROV -COD8YL-CSPROVYD-CSPROVYE-CSPROVYF-CSPROVYG-CSPROVYH-COD8YT) - SEUIL_61) * 2)
	+ ((1-positif_ou_nul((TOTCR - CSGIM - CRDSIM - PRSPROV - CDISPROV -COD8YL-CSPROVYD-CSPROVYE-CSPROVYF-CSPROVYG-CSPROVYH-COD8YT) - SEUIL_61))*positif(TOTCR) * 1)
	+ (null(TOTCR) * 3);
regle 9070031  :
application :  iliad, batch ;
...
INDREV1A8IR = positif (
 ...
 + positif( HEURESUPV ) + positif (LOCDEFNPC) 
 + positif( LOCDEFNPCGAC)
 + positif( LOCDEFNPCGAPAC) 
 + positif( LOCDEFNPCGAV)
 + positif( LOCDEFNPPAC)
 + positif( LOCDEFNPV) 
 + positif( LOCDEFPROC) 
 ...
 + positif( COD3VE ) + positif( COD3VV ) + positif( COD3WI ) + positif( COD3WJ )
 ...
 + present( INVLOCHOTR1 ) + present( INVLOG2008 )
 ...
 + present( PATNAT ) + present( PATNAT1 ) + present( PATNAT2 ) + present( PATNAT3 )
 ...
 + present( CODHSA ) + present( CODHSB ) + present( CODHSC ) + present( CODHSD ) + present( CODHSE )
 + present( CODHSF ) + present( CODHSG ) + present( CODHSH ) + present( CODHSI ) + present( CODHSJ )
 + present( CODHSK ) + present( CODHSL ) + present( CODHSM ) + present( CODHSN ) + present( CODHSO )
 + present( CODHSP ) + present( CODHSQ ) + present( CODHSR ) + present( CODHSS ) + present( CODHST )
 + present( CODHSU ) + present( CODHSV ) + present( CODHSW ) + present( CODHSX ) + present( CODHSY )
 + present( CODHSZ ) + present( CODHTA ) + present( CODHTB ) + present( CODHTC ) + present( CODHTD ) 
...
RETX = positif(CMAJ) * TXINT
       ...
       + TXINR * positif(TXINR_A) * null(TXINR - TXINR_A)) * (1-positif(TINR_1)
               * positif(INRIR_NET_1+INRCSG_NET_1+INRRDS_NET_1+INRPRS_NET_1+INRCDIS_NET_1
                         ...
                         +INRRSE1_NET_1+INRRSE2_NET_1+INRRSE3_NET_1+INRRSE4_NET_1+INRRSE5_NET_1
                                                              ))
       + (-1) * positif(TINR_1) * positif(INRIR_NET_1+INRCSG_NET_1+INRRDS_NET_1+INRPRS_NET_1+INRCDIS_NET_1+INRTAXA_NET_1
                        +INRCGLOA_NET_1
                                                              )
    ;
...
regle  90716 :
application: batch , iliad ;
WMTAP =  (1 - positif(V_ZDC+0)) * positif(NAT1 + NAT71)* max(0,IINET - HAUTREVNET);
...
IMPNETCS = (NAPCRP - TOTCRA) * null(4 - V_IND_TRAIT)
           + (NAPCS - V_CSANT) * null(5 - V_IND_TRAIT);
IMPNETRD = NAPRD - V_RDANT ;
IMPNETPRS = NAPPS - V_PSANT ;
IMPNETCVN = NAPCVN - V_CVNANT ;
...
INDSEUIL12 = positif_ou_nul(max(0 , CSNET+RDNET+PRSNET+CVNNET+CDISNET+CGLOANET+RSE1NET+RSE2NET+RSE3NET+RSE4NET+RSE5NET )- SEUIL_12);
...
regle 9101352:
application : iliad ,batch;
VARPS61 =  CSG + RDSN + PRS + PCSG + PRDS + PPRS + CVNSALC + PCVN + CDIS + PCDIS + CGLOA + PGLOA + RSE1N + PRSE1 + RSE2N + PRSE2 + RSE3N + PRSE3 + RSE4N + PRSE4 + RSE5N + PRSE5
           - CSGIM - CRDSIM - PRSPROV - COD8YT - CDISPROV -COD8YL-CSPROVYD-CSPROVYE-CSPROVYF-CSPROVYG-CSPROVYH+0 ;
...
TOTCRBIS = si (
           ...
           finsi;
TOTCR = si ( (TOTCRBIS - CSGIM - CRDSIM - PRSPROV - COD8YT - CDISPROV -COD8YL-CSPROVYD-CSPROVYE-CSPROVYF-CSPROVYG-CSPROVYH < SEUIL_61)
             et (CSGIM + CRDSIM + PRSPROV + COD8YT + CDISPROV +COD8YL+CSPROVYD+CSPROVYE+CSPROVYF+CSPROVYG+CSPROVYH+0>0) )
        alors (CSGIM + CRDSIM + PRSPROV + COD8YT + CDISPROV +COD8YL+CSPROVYD+CSPROVYE+CSPROVYF+CSPROVYG+CSPROVYH+0)
        sinon (TOTCRBIS+0)
        finsi;
...
regle 905163 :
application : iliad,batch ;
NAPCRP = max(0 , CSNET+RDNET+PRSNET+CVNNET+CDISNET+CGLOANET+RSE1NET+RSE2NET+RSE3NET+RSE4NET+RSE5NET );
NAPCRTOT = NAPCRP;
regle 9051631 :
application : iliad,batch ;
...
NAPCRINR = null(4-V_IND_TRAIT)
        ...
        + null(5-V_IND_TRAIT)
               * max(0 , (CSTOT - CSGIM - CRDSIM - PRSPROV - CDISPROV -COD8YL-CSPROVYD-CSPROVYE-CSPROVYF-CSPROVYG-CSPROVYH-COD8YT) );
NAPCR61 = NAPCS + NAPRD +NAPPS +NAPCVN +NAPCDIS +NAPGLOA +NAPRSE1 +NAPRSE2 +NAPRSE3 +NAPRSE4  +NAPRSE5;
regle 905165 :
application : iliad,batch ;
...
TXMOYIMPDEN1 =   somme (i=V,C,1,2,3,4: TSNTi) * (1-positif(abs(DRBG)))
	...
	+ PLOCNETF + max(0,NPLOCNETF)
	  ...
	  + (BAHQTOT * positif(BAHQTOT)) * (1-positif(abs(DRBG))) * null(DEFBA6+DEFBA5+DEFBA4+DEFBA3+DEFBA2+DEFBA1)
	  + (BAHQT * (1-positif(BAHQT))* (1-positif(BAQTOT))) * (1-positif(abs(DRBG)))
	 ...
	 * null(BNCDF1 + BNCDF2 +BNCDF3 +BNCDF4 +BNCDF5 +BNCDF6) * (1-positif(abs(DRBG)))
         + (BPVRCM + PVTAXSB + PVIMPOS + COD3WI + BPVKRI) * (1 - positif( IPVLOC ))* (1-positif(present(TAX1649)+present(RE168)))
                ;
...
TXMOYIMPDEN3 = (
		...
		+ (BPTPVT +PVSURSIWG+BPTPSJ)* (1 - positif( IPVLOC ))* (1-positif(present(TAX1649)+present(RE168)))
                + (BTP2 + BPTP4    + BTP40 + BTP18  + COD3WJ)* (1 - positif( IPVLOC ))* (1-positif(present(TAX1649)+present(RE168)))
                + RCM2FA 
               )
               * (1 - positif(IPVLOC)));
